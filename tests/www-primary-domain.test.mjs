...
test("astcompass.com is canonical across runtime and static metadata", () => {
  ...
  assert.match(robots, /Sitemap: https:\/\/astcompass\.com\/sitemap\.xml/);
});

test("apex/www forwarding to Netlify while retaining the Netlify-host redirect", () => {
  const redirects = read("public/_redirects");
  const netlify = read("netlify.toml");
  assert.doesNotMatch(redirects, /^https?:\/\/(?:www\.)?astcompass\.com\/\*/m);
  assert.match(redirects, /https:\/\/astcompass\.netlify\.app\/\* https:\/\/astcompass\.com\/:splat 301!/);
  assert.doesNotMatch(netlify, /from = "https?:\/\/(?:www\.)?astcompass\.com\/\*"/);
  assert.match(netlify, /from = "https:\/\/astcompass\.netlify\.app\/\*"[\s\S]*?to = "https:\/\/astcompass\.com\/:splat"/);
