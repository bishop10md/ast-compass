...
test("public analyses are session-only and account-free", () => {
  ...
  assert.match(read("src/features/Feedback.tsx"), /accountStatus:\s*signedIn\s*\?\s*"Authenticated"\s*:\s*"Guest"/);
  assert.match(read("supabase/migrations/20260902_private_feedback.sql"), /account_status='Guest'/);
  assert.match(read("src/services/feedbackService.ts"), /app_version:\s*APP_VERSION/);
});
