...
import { submitFeedback, FeedbackTimeoutError, type FeedbackCategory } from "../services/feedbackService";
...
export default function Feedback({ onReturn }: { onReturn: () => void }) {
  ...
  const [status, setStatus] = useState<"idle" | "sending" | "success" | "error" | "unconfirmed">("idle");
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setStatus(error instanceof FeedbackTimeoutError ? "unconfirmed" : "error");
    }
  };
