...
// Availability is an operational guard, not evidence of scientific review.
type SignatureAvailability =
  | { availability: "active" }
  | { availability: "paused-pending-review"; reviewStatus: "Draft"; pauseReason: string };

export const quinolonePauseReason = "Quinolone mechanism inference is currently unavailable pending scientific review. Quinolone resistance may involve multiple overlapping mechanisms, and the observed AST pattern alone does not establish a specific molecular mechanism.";
...
export type PhenotypeAbstention = {
  signatureId: string; availability: "paused-pending-review"; reviewStatus: "Draft";
  explanation: string; sourceIds: string[];
};
export type PhenotypeAnalysis = { organismId: string; reviewedRows: number; intrinsicFindings: string[]; candidates: PhenotypeCandidate[]; abstentions: PhenotypeAbstention[]; insufficient: boolean; combinedPossible: boolean };
...
export const phenotypeSignatures: PhenotypeSignature[] = [
  {id:"esbl-like",availability:"active",organisms:enterobacterales,mechanismId:"esbl",candidateMarkerIds:["CTX-M"],expectedPatterns:[
  ...
  {id:"ampc-like",availability:"active",organisms:ampcRisk,mechanismId:"ampc",expectedPatterns:[
  ...
  {id:"carbapenemase-like",availability:"active",organisms:[...enterobacterales,"pseudomonas","acinetobacter"],mechanismId:"serine_carb",candidateMarkerIds:["KPC","NDM","VIM","IMP","OXA-48-like"],expectedPatterns:[
  ...
  {id:"mbl-like",availability:"active",organisms:[...enterobacterales,"pseudomonas","acinetobacter"],mechanismId:"mbl",candidateMarkerIds:["NDM","VIM","IMP"],expectedPatterns:[
  ...
  {id:"mrsa-like",availability:"active",organisms:["staph_aureus","cons","slugdunensis"],mechanismId:"pbp2a",candidateMarkerIds:["mecA","mecC"],expectedPatterns:[
  ...
  {id:"vre-like",availability:"active",organisms:["efaecalis","efaecium"],mechanismId:"van_target",candidateMarkerIds:["vanA","vanB"],expectedPatterns:[
  ...
  {id:"mlsb-like",availability:"active",organisms:["staph_aureus","cons","slugdunensis","spneumo","bhs","vgs"],mechanismId:"mlsb",expectedPatterns:[
  ...
  {id:"quinolone-like",availability:"paused-pending-review",pauseReason:quinolonePauseReason,organisms:organisms.filter(o=>o.gram!=="fungal"&&o.gram!=="acid-fast").map(o=>o.id),mechanismId:"quinolone",expectedPatterns:[
...
/** Disclose unavailable inference even for S/Unknown rows; never interpret their category. */
export function getPhenotypeAbstentions(organismId: string, input: AstPhenotypeRow[]): PhenotypeAbstention[] {
  return phenotypeSignatures.flatMap(signature => {
    if (signature.availability !== "paused-pending-review" || !applicable(signature, organismId)) return [];
    const relevant = input.some(row => {
      const drug = antibiotics.find(item => item.id === row.antimicrobialId);
      return !!drug && signature.expectedPatterns.some(pattern =>
    ...
    return relevant ? [{ signatureId: signature.id, availability: signature.availability, reviewStatus: signature.reviewStatus, explanation: signature.pauseReason, sourceIds: [...signature.sourceIds] }] : [];
  });
}
...
export function analyzePhenotype(organismId:string,input:AstPhenotypeRow[]):PhenotypeAnalysis{
 ...
 const scored=phenotypeSignatures.filter(s=>s.availability==="active"&&applicable(s,organismId)).map(signature=>{let score=0;const supporting:string[]=[],against:string[]=[];let evidence=0;
 ...
 return{organismId,reviewedRows:rows.length,intrinsicFindings,candidates:scored,abstentions:getPhenotypeAbstentions(organismId,input),insufficient:scored.length===0,combinedPossible:scored.filter(c=>c.compatibility==="Highly compatible"||c.compatibility==="Compatible").length>1};
}
