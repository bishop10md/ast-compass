...
const scaleFor = (record: Breakpoint | undefined, drug: Antibiotic, organism: Organism): TeachingScale | null => {
  if (record && (!record.availability || record.availability === "Clinical breakpoint available")) return { susceptibleMax: numberFrom(record.susceptible, 1), resistantMin: numberFrom(record.resistant, 4), unit: record.unit, origin: "Loaded demo record", note: record.footnote };
...
export default function BreakpointEngine() {
  ...
  return <>
    ...
    <div className="workspace-grid"><aside className="filter-panel"><SearchableSelect label="Organism" value={organism?.id || ""} onChange={selectOrganism} options={domainOrganisms.map((item) => ({ value: item.id, label: item.name, aliases: item.aliases, description: item.group }))} placeholder="Search organism or alias…"/><SearchableSelect label="Applicable antimicrobial" value={drug?.id || ""} onChange={setDrugId} options={domainDrugs.map((item) => ({ value: item.id, label: item.name, aliases: [item.short], description: item.className }))} placeholder="Search antimicrobial or class…"/><div className="isolate-simulator"><b>Generate a simulated isolate</b><label>Population profile<select value={isolateProfile} onChange={(e) => setIsolateProfile(e.target.value as typeof isolateProfile)}><option value="typical">Typical / wild-type-like</option><option value="acquired">Acquired resistance</option><option value="intrinsic">Intrinsic resistance</option></select></label><button type="button" onClick={generateIsolate}>Generate two-fold MIC</button><small>{simulationNote}</small></div><label>MIC ({scale?.unit || "units"})<input className="field mic-input" type="number" min="0" step="any" value={mic} onChange={(e) => { setMic(e.target.value); setSimulationNote("Manual MIC entry"); }} placeholder="Enter any MIC"/></label><div className="selection-summary"><span>Organism group</span><b>{organism?.group}</b><span>Drug class</span><b>{drug?.className}</b><span>Teaching source</span><b>{scale?.origin}</b></div></aside>
      {scale && drug ? <section className="panel result-panel"><div className="result-title"><div><p className="eyebrow">{organism?.name} · {drug.name}</p><h2>MIC interpretation simulation</h2><span>{record?.method || "MIC"} · {scale.unit} · {scale.origin}</span></div><span className="demo-stamp">NOT CLINICAL</span></div>{record?.availability && <div className={`criteria-status ${record.availability === "Clinical breakpoint available" ? "available" : "unavailable"}`}><span>Selected-standard record status</span><b>{record.availability}</b><p>{record.categoryMeaning}</p></div>}<div className="breakpoint-scale"><div className="sus"><span>S</span><b>≤ {scale.susceptibleMax}</b><small>Teaching susceptible</small></div><div className="int"><span>I / SDD</span><b>&gt; {scale.susceptibleMax} to &lt; {scale.resistantMin}</b><small>{standard === "EUCAST" ? "Susceptible, increased exposure" : "Intermediate / SDD"}</small></div><div className="res"><span>R</span><b>≥ {scale.resistantMin}</b><small>Teaching resistant</small></div></div>{interpretation ? <div className={`mic-answer ${interpretation.tone}`}><span>Generated teaching interpretation</span><h3>{interpretation.category}</h3><p>{interpretation.explanation}</p></div> : <div className="mic-empty">Enter any valid non-negative MIC to generate a teaching interpretation.</div>}<div className="pattern-explainer"><b>How to reason about this organism’s resistance patterns</b><p>{patternFor(organism?.group, organism?.name)}</p><small>The category describes the simulated phenotype. It does not prove a mechanism or predict patient outcome.</small></div><div className="note"><b>{scale.origin}</b><p>{scale.note}</p>{record && <><p><strong>Standard/version:</strong> {record.standard} · {record.version}</p><p><strong>Last verified:</strong> {record.lastVerified || record.meta.lastReviewed}</p><p><strong>Restrictions:</strong> {record.siteRestriction} · {record.specimenRestriction || "No specimen restriction recorded"}</p><a href={refFor(record.sourceId)?.url} target="_blank" rel="noreferrer">Open supporting source ↗</a></>}</div></section> : <section className="no-breakpoint"><span>∅</span><div><h2>{record?.availability || "No applicable teaching combination"}</h2><p>{record?.footnote || "This organism domain currently has no mapped antimicrobial options."}</p>{record && <><p>{record.dosageNote}</p><a href={refFor(record.sourceId)?.url} target="_blank" rel="noreferrer">Open supporting source ↗</a></>}</div></section>}
    </div>
  </>;
}
