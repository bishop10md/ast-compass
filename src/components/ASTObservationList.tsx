...
import type { SearchOption } from "../utils/search";
...
export default function ASTObservationList({ rows, options, onChange, onAdd, onRemove, addLabel = "+ Add antimicrobial", maxRows = 20 }: Props) {
  ...
  return <div className="ast-observation-list">
    ...
    {rows.map((row, index) => <fieldset className="ast-observation-row" aria-label={`Antimicrobial result ${index + 1}`} key={row.id}>
      ...
      <SearchableSelect label="Antimicrobial" value={row.antimicrobialId} onChange={value => onChange(row.id, { antimicrobialId: value })} options={options.filter(option => !selected.has(option.value) || option.value === row.antimicrobialId)} placeholder="Search name or abbreviation…"/>
